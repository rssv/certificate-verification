...
const ViewAllResults = (props) => {
    const [viewSem, setViewSem] = useState(0);
    const viewSemClickHandler = (event) => {
        setViewSem(event.target.value);
    }
    return (
        <div className={classes.backdrop}>
            <div className={classes.modal}>
                <div className={classes.sem_buttons}>
                    {props.studentData.results.map((r, i) => {
                        return <button value={i} key={i} onClick={viewSemClickHandler}>{`Semester${r.semNo}`}</button>
                    })}
                </div>
