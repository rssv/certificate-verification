...
const ViewMarks = (props) => {
    ...
    const fetchStudentList = async (accessToken) => {
        let response;
        try{
            response= await fetch(`http://localhost:4000/student/instructor/${ctx.user.username}/submitted`,{
                method: 'GET',
                headers: {
    ...
    return (
        <div className={classes.contents}>
            ...
            {!!studentListDisplay.list.length && <div className={classes.marks_form}>
                  ...
                  {studentListDisplay.list.map((student, i)=>{
                      return (
                        <tr className={classes.row} key={i}>
                            ...
                            <td className={classes.data}>{student.subjects.marks}</td>
                        </tr>
                      )
                  })}
                  </tbody>
