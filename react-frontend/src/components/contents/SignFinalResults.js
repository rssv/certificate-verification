...
const SignFinalResults = () => {
    ...
    const fetchSignedResults = async (accessToken) => {
        let response;
        try{
            response= await fetch("http://localhost:4000/results/final-approved",{
                method: 'GET',
                headers: {
    ...
    const fetchStorehashFinal = async (accessToken, bodyData) => {
        let response;
        try{
            response= await fetch("http://localhost:4000/result-final/storeHash-new", {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'authorization': `Bearer ${accessToken}`
    ...
    const signHandler = (event) => {
        ...
        console.log(data);
        fetchStorehashFinal(ctx.user.accessToken, data).then(() => {
        ...
        .catch(async(err) => {
            ...
            if(err.message === "jwt expired"){
                let token = await ctx.refreshToken();
                fetchStorehashFinal(token, data).then(() => {
                    setStudentListDisplay((prev) => {
                        let newState = prev.filter((s) => {
                            return (s.admNo !== viewResult.data.admNo)
                        })
    ...
    return (
        <React.Fragment>
            {viewResult.flag && <ViewAllResults studentData={viewResult.data} approveHandler={signHandler} denyHandler={denyHandlerViewResult} showDeny={true} buttonName={buttonName}/>}
...
export default SignFinalResults;